function plot_hierarchical_similarity(Group)
...
for i = 1:nCi
    for ii = 1:nCi
        ...
        FCsim(i,ii)= corr(group_FC(:,i),group_FC(:,ii));
    end
end
%% Hierarchical clustering on FC profile
D = 1-FCsim; 
for ii = 1:length(D),D(ii,ii) = 0;end % dirty fix for some rounding error makes diagonal not zero
Z = linkage(squareform(D),'complete'); % Update 2025.06.05, seems that we did not pass the dissimilarity matrix as a distance so linkage might have calculated a Euclidean distance on this matrix before
c = cophenet(Z,D)
...
GroupPct = repelem(1/size(FCsim,2),size(FCsim,2));
xticks([]);
set(gca,'Fontsize',12);
% bottom subplot;
subaxis(10,10,92:1:100,'MB',0.05,'MT',0.05,'ML',0.025,'MR',0.075);
...
% sweep through
% the networks
for i = 1:size(FCsim,2)
    Tmp(i).FaceColor = sorted_colors(i,:);
end
print('Dendrogram_ColorLabels.png','-dpng','-r300');
...
set(gca,'Fontsize',11);
print('Dendrogram_Numbers.png','-dpng','-r300');

%% Get Clusters at different hierarchical levels
n = size(Z, 1) + 1;  % number of original items
cluster_labels_all = zeros(n, n-1);  % preallocate

for i = 1:n-1
    cluster_labels_all(:, i) = cluster(Z, 'maxclust', n - i + 1);
end

cluster_labels_all = postprocess_ordinal_multilayer(cluster_labels_all);
save('hierarchical_cluster_labels_FCsim.mat','cluster_labels_all');

%% Steal code from the Lynch 2024 PFM repo to plot the sorted heatmap with colors
...
print(gcf,['FCsim_btwn_Communities_hierarchical.png'],'-dpng','-r300');
close;
%% Plot legend
figure('Units','inches','position',[10 10 8 4])%[10 10 5,3]);%[10 10 5 2]
...
legend(Group.NetworkLabels(sort_idx),'interpreter','none','FontSize',10,'location','best','Orientation','vertical','NumColumns',3);
legend('boxoff')
xlim([10,11]);
axis('off')
print('networks_Legend','-dpng','-r300');
end
