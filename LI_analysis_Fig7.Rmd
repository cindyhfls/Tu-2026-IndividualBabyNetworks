...
# sum(!is.na(data$VDQ))       # non-missing VDQ
# sum(!is.na(data$NVDQ))      # non-missing NVDQ
# sum(!is.na(data$age_yrs))   # non-missing age
# sum(!is.na(data$age_yrs) & !is.na(data$VDQ) & !is.na(data$NVDQ))
```
## BCP (Dataset 2, Figure 6)
```{r}
data <-read.csv("LI_table_for_BCP.csv")
nrow(data) # should be 280
data$sub_id <- factor(data$sub_id)
data$LI <-data$LI_1 # change here for network p
# Scale the variables
data$LI_scaled <- scale(data$LI)
data$age_scaled <- scale(data$age_yrs)
data$VDQ_scaled <- scale(data$mullen_VDQ)   
data$NVDQ_scaled <- scale(data$mullen_NVDQ) 
```
```{r}
...
# standardized
m_age_lang_scaled <- lmer(LI_scaled ~ age_scaled + (1 | sub_id), data = data)
summary(m_age_lang_scaled)
r2(m_age_lang_scaled)
```
...
m_lang <- lmer(
  LI ~ age_yrs + mullen_NVDQ + (1 | sub_id),
  data = data
)
summary(m_lang)
r2(m_lang)
...
m_lang_scaled <- lmer(
  LI_scaled ~ age_scaled + NVDQ_scaled + (1 | sub_id),
  data = data
)
summary(m_lang_scaled)
r2(m_lang_scaled)
```
```{r}
# outlier?
...
data$LI_scaled <- scale(data$LI)
data$age_scaled <- scale(data$age_yrs)
data$langcomp_scaled <- scale(data$langcomp)   
data$motcomp_scaled <- scale(data$motcomp) 
```
```{r}
...
m_age_lang <- lmer(LI_scaled ~ age_scaled + (1 | sub_id), data = data)
summary(m_age_lang)
```
```{r}
# outlier?
...
m_lang <- lmer(
  LI_scaled ~ age_scaled + langcomp_scaled + (1 | sub_id),
  data = data
)
summary(m_lang)
```
